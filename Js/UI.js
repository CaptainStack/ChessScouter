...
function getBackgroundImageString(position) {
  ...
  if (game.board.getPiece(position.x, position.y) != null) {
    if (dot !== "" || fork !== "") {
      piece = ", url(Assets/" + game.board.getPiece(position.x, position.y).imageFileName() + ")";
    } else{
      piece = "url(Assets/" + game.board.getPiece(position.x, position.y).imageFileName() + ")";
    }
  }
...
function getImage(x, y) {
  if (game.board.getPiece(x, y) !== null && !game.board.getPiece(x, y).captured) {
      return game.board.getPiece(x, y).imageFileName();
  } else {
      return "";
  }
};
...
function inputFen(){
  var fenString = document.getElementById('txtInputFen').value;
  if (!Fen.validate(fenString)){
    alert('Invalid FEN');
  }else{
    newGame(fenString);
  }
